...
import { TextField } from '../common/FormField.jsx'
import * as api from '../../services/api.js'

const ERROR_MESSAGES = {
  not_found: 'No account with that email exists.',
  cannot_share_with_self: 'You already own this group.',
...
export default function GroupShareModal({ open, onClose, groupId }) {
  ...
  async function handleAdd() {
    ...
    setError('')
    try {
      const collaborator = await api.addGroupCollaborator(groupId, { email: email.trim(), role })
  ...
  async function handleRoleChange(collaborator, newRole) {
    const updated = await api.updateGroupCollaboratorRole(groupId, collaborator.id, newRole)
    setCollaborators((list) => list.map((c) => (c.id === collaborator.id ? updated : c)))
  }

  async function handleRemove(collaborator) {
    await api.removeGroupCollaborator(groupId, collaborator.id)
  ...
  return (
    <Modal open={open} onClose={handleClose} maxWidth={520}>
      <ModalHeader title="Share group" onClose={handleClose} />
      <div className="text-sm text-text-secondary mt-1.5">
        Invite a registered user by email. Editors can chat and edit BRDs in this group; viewers can only view and export.
      </div>
      ...
      <ConfirmModal
        ...
        title="Remove access?"
        description={removeTarget ? `${removeTarget.name} will no longer be able to view or edit this group.` : ''}
        confirmLabel="Remove"
      />
    </Modal>
  )
